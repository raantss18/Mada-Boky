...
\begin{visualisation}[réunion, intersection, contraire]
\begin{center}
\begin{tikzpicture}[font=\footnotesize]
  \draw[bmtGris, line width=0.8pt] (-2.4,-1.5) rectangle (2.4,1.5);
  \node[bmtGris, anchor=north east] at (2.35,1.45) {$\Omega$};
  \begin{scope}
    \clip (-2.4,-1.5) rectangle (2.4,1.5);
    \fill[bmtIndigo, opacity=0.18] (-0.75,0) circle (1.35);
    \fill[bmtLaterite, opacity=0.18] (0.75,0) circle (1.35);
    \fill[bmtVetiver, opacity=0.35] (-0.75,0) circle (1.35)
      (0.75,0) circle (1.35);
  \end{scope}
  \draw[bmtIndigo, line width=1pt] (-0.75,0) circle (1.35);
  \draw[bmtLaterite, line width=1pt] (0.75,0) circle (1.35);
  \node[bmtIndigo] at (-1.55,0.95) {$A$};
  \node[bmtLaterite] at (1.55,0.95) {$B$};
  \node[bmtVetiver, font=\footnotesize\bfseries] at (0,0) {$A \cap B$};
  \node[bmtGris, anchor=north, align=center] at (0,-1.85)
    {$A \cup B$ : la zone teintée, comptée une seule fois};
\end{tikzpicture}
\end{center}
La formule $\proba{A \cup B} = \proba{A}+\proba{B}-\proba{A \cap B}$ se lit
sur ce dessin : en additionnant simplement $\proba{A}$ et $\proba{B}$, la
zone commune $A \cap B$ est comptée deux fois, il faut donc la retrancher
une fois. Le contraire $\evtc{A}$ est tout ce qui, dans le rectangle $\Omega$,
reste en dehors du cercle $A$.
\end{visualisation}
...
\begin{corrige}
\textbf{1.} Le tirage est simultané : on compte des parties. Il y a
$\combi{2}{10} = 45$ tirages possibles et $\combi{2}{4} = 6$ favorables, d'où
une probabilité de $\dfrac{6}{45} = \dfrac{2}{15}$.

\textbf{2.} Avec remise, les deux tirages sont indépendants et identiques :
la probabilité vaut $\left(\dfrac{4}{10}\right)^{2} = \dfrac{4}{25}$.

\textbf{3.} On passe par l'événement contraire, « aucun $6$ », de probabilité
$\left(\frac56\right)^{3}$ : la réponse est
$1-\dfrac{125}{216} = \dfrac{91}{216}$.

\textbf{4.} Sur les six faces, quatre portent un nombre premier — le $2$ et
les trois $3$ — donc la probabilité d'un numéro premier vaut $\frac23$ à
chaque lancer. Les trois lancers étant indépendants, la réponse est
$\left(\frac23\right)^{3} = \dfrac{8}{27}$.

\textbf{5.} Il y a $\combi{2}{9} = 36$ tirages. La somme $b$ prend les valeurs
$2$, $3$, $4$, $5$ et $6$, avec
\[
  \proba{b=2} = \frac{6}{36}, \quad
  \proba{b=3} = \frac{12}{36}, \quad
  \proba{b=4} = \frac{11}{36}, \quad
  \proba{b=5} = \frac{6}{36}, \quad
  \proba{b=6} = \frac{1}{36},
\]
la valeur $4$ s'obtenant de deux façons : un $1$ et un $3$, ou deux $2$. La
somme des cinq probabilités vaut bien $1$.
\end{corrige}
...
La probabilité qu'un élève soit reçu sachant qu'il a travaillé, et celle qu'il
ait travaillé sachant qu'il est reçu, sont deux nombres différents. Confondre
les deux est l'erreur la plus coûteuse du chapitre.
\end{avertissement}
...
\begin{corrige}
\textbf{6.} $\probacond{B}{A} = \dfrac{\proba{A \cap B}}{\proba{A}}
= \dfrac{0{,}1}{0{,}4} = 0{,}25$.

\textbf{7.} On conditionne sur la couleur transférée. Si elle est blanche —
probabilité $\frac34$ —, la seconde urne contient deux blanches et une noire,
et la probabilité de tirer blanc vaut $\frac23$. Si elle est noire —
probabilité $\frac14$ —, cette probabilité tombe à $\frac13$. La formule des
probabilités totales donne
$\frac34 \times \frac23+\frac14 \times \frac13 = \dfrac{7}{12}$.

\textbf{8.} Notons $C$ « être porteur » et $T$ « être testé positif ». Alors
\[
  \proba{T} = 0{,}02 \times 0{,}95+0{,}98 \times 0{,}03 = 0{,}0484,
\]
puis $\probacond{C}{T} = \dfrac{0{,}02 \times 0{,}95}{0{,}0484}
= \dfrac{0{,}019}{0{,}0484} \approx 0{,}393$. Moins de deux personnes
positives sur cinq sont réellement porteuses : c'est la rareté du caractère,
et non la qualité du test, qui produit ce résultat contre-intuitif.

\textbf{9.} Le gibier échappe si les deux manquent, ce qui a pour probabilité
$\frac14 \times \frac25 = \frac{1}{10}$. Il est donc touché avec la
probabilité $\dfrac{9}{10}$.
\end{corrige}
...
\begin{corrige}
\textbf{10.} $\proba{A} \times \proba{B} = 0{,}5 \times 0{,}4 = 0{,}2$, qui
est bien égal à $\proba{A \cap B}$ : les deux événements sont indépendants.

\textbf{11.} La probabilité d'obtenir au moins un $6$ vaut
$1-\left(\frac56\right)^{n}$, et la condition s'écrit
$\left(\frac56\right)^{n}<0{,}1$, soit
$n > \dfrac{\ln 10}{\ln 1{,}2} \approx 12{,}6$. Le premier entier convenable
est $n = 13$.

\textbf{12.} Ne jamais réaliser l'événement a pour probabilité
$\left(\frac23\right)^{n}$, et l'inéquation
$\left(\frac23\right)^{n}<10^{-3}$ donne
$n > \dfrac{3\ln 10}{\ln 1{,}5} \approx 17{,}0$. On vérifie que
$\left(\frac23\right)^{17} \approx 1{,}02\times10^{-3}$ dépasse encore le
seuil : le premier entier convenable est $n = 18$.
\end{corrige}
...
tirer au hasard une boule. Calculer $\proba{A}$, $\proba{B}$ et
$\proba{C}$, puis, en répétant cinq fois l'épreuve de manière indépendante, la
probabilité de l'événement $F$ : « la lettre $B$ apparaît exactement deux
fois ».
...
\exoann{Concours d'entrée, ENI Fianarantsoa, session 2008-2009 — exercice 2}
Deux chasseurs aperçoivent ensemble un gibier et tirent simultanément. Le
premier atteint et tue d'habitude trois canards sur quatre, le second trois
sur cinq.
\begin{enumerate}[label=\textbf{\alph*)}, leftmargin=*, itemsep=0.15em]
  \item Quelle est la probabilité pour que le gibier soit tué ?
  \item En fait le second a tiré le premier ; s'il tire et manque, les chances
        du premier sont diminuées de moitié. Quelle est la probabilité que le
        premier tue le canard ?
  \item Dans ces conditions, quelle est la probabilité que le canard en
...
\textbf{4.} $\proba{A} = \frac{3}{10}$, $\proba{B} = \frac{5}{10} = \frac12$,
$\proba{C} = \frac{2}{10} = \frac15$. Les cinq épreuves étant indépendantes,
\[
  \proba{F} = \combi{2}{5}\left(\frac12\right)^{2}\left(\frac12\right)^{3}
  = 10 \times \frac{1}{32} = \frac{5}{16}.
\]

\textbf{5.} Les sommes possibles et leurs probabilités, obtenues par tirage
...
\textbf{6.} Les multiples de trois sur le dé sont $3$ et $6$, de probabilité
$\frac13$. Donc $p_{n} = \left(\frac23\right)^{n}$, qui tend vers $0$.

\textbf{7.} L'arbre a deux niveaux, les probabilités du second dépendant du
...
\textbf{8.} $\proba{R} = 0{,}75$, $\probacond{\evtc{E}}{\evtc{R}} = 0{,}08$,
...
\textbf{9. a)} Le gibier survit seulement si les deux manquent :
$1-\frac14 \times \frac25 = 1-\frac{1}{10} = \frac{9}{10}$.
\textbf{b)} Si le second touche — probabilité $\frac35$ —, le premier garde sa
probabilité $\frac34$ ; s'il manque — probabilité $\frac25$ —, elle tombe à
$\frac38$. Donc
$\frac35 \times \frac34+\frac25 \times \frac38 = \frac{9}{20}+\frac{3}{20}
= \frac35$.
\textbf{c)} Le canard en réchappe si le second manque puis le premier aussi :
$\frac25 \times \left(1-\frac38\right) = \frac25 \times \frac58 = \frac14$.

\textbf{10. a)} $P_{0} = \frac16$, $P_{1} = \frac13$, $P_{2} = \frac12$.
...
\begin{enumerate}[label=\textbf{\arabic*.}, leftmargin=*, itemsep=0.25em]
  ...
  \item On répète cinq fois de suite et de manière indépendante l'épreuve
        $(E)$.
        Calculer la probabilité de l'événement $F$ : « la lettre $B$
        apparaît exactement deux fois ». \hfill\textup{(0,5 pt)}
\end{enumerate}

\medskip
\bmtsource{La dernière question du sujet fait intervenir une variable
aléatoire : elle figure au chapitre 18.}
\end{commeaubac}
...
\textbf{2.} Les cinq épreuves sont identiques et indépendantes, avec
...
Trois machines $M_{1}$, $M_{2}$, $M_{3}$ fabriquent respectivement $50\,\%$,
$30\,\%$ et $20\,\%$ des pièces d'un atelier. Les taux de pièces défectueuses
sont respectivement $1\,\%$, $2\,\%$ et $5\,\%$.

\begin{enumerate}[label=\textbf{\arabic*.}, leftmargin=*, itemsep=0.25em]
  \item Calculer la probabilité qu'une pièce prise au hasard soit
        défectueuse.
  \item Une pièce est défectueuse : calculer la probabilité qu'elle provienne
        de $M_{3}$.
  \item L'atelier remplace $M_{3}$ par une machine dont le taux de défauts est
        $t$. Déterminer $t$ pour que la probabilité globale de défaut
        descende sous $1{,}2\,\%$.
\end{enumerate}
\end{concours}

\begin{corrige}[vers le concours]
\textbf{1.} $\proba{D} = 0{,}5 \times 0{,}01+0{,}3 \times 0{,}02
+0{,}2 \times 0{,}05 = 0{,}005+0{,}006+0{,}010 = 0{,}021$.

\textbf{2.} $\probacond{M_{3}}{D} = \frac{0{,}010}{0{,}021}
\approx 0{,}476$ : près d'une pièce défectueuse sur deux vient d'une machine
qui n'assure qu'un cinquième de la production.

\textbf{3.} La probabilité devient $0{,}011+0{,}2\,t$, et l'on veut
$0{,}011+0{,}2\,t<0{,}012$, soit $t<0{,}005$ : il faut une machine dont le
taux de défauts reste sous un demi pour cent.
\end{corrige}
...
\exods{1}{Baccalauréat, Côte d'Ivoire, série C, session 2023 — exercice 1}{5 points}
...
\exods{2}{Baccalauréat, Madagascar, série S, session 2022 — exercice 2}{7 points}
...
\exods{3}{Concours d'entrée, ENI Fianarantsoa, session 2008-2009 — exercice 2}{8 points}
Deux chasseurs aperçoivent ensemble un gibier et tirent simultanément. Le
premier atteint et tue d'habitude trois canards sur quatre, le second trois
sur cinq.
\begin{enumerate}[label=\textbf{\alph*)}, leftmargin=*, itemsep=0.15em]
...
\textbf{Exercice 1.} Vraie. Le vecteur $(2\,;3\,;4)$, lu sur l'équation, est normal
au plan, et $(2\,;3\,;4) = 2\left(1\,;\frac32\,;2\right)$ : les deux vecteurs
sont colinéaires, donc $\vecn$ est également normal au plan.

\textbf{Exercice 2.} $\vect{AB}(-3\,;-2\,;0)$ et $\vect{AC}(-5\,;-2\,;-1)$, donc
...
\textbf{Exercice 3. a)} Le gibier survit seulement si les deux manquent :
$1-\frac14 \times \frac25 = 1-\frac{1}{10} = \frac{9}{10}$.
\textbf{b)} Si le second touche — probabilité $\frac35$ —, le premier garde sa
probabilité $\frac34$ ; s'il manque — probabilité $\frac25$ —, elle tombe à
$\frac38$. Donc
