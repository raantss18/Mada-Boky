...
\begin{tikzpicture}[x=1.1cm, y=1.1cm, font=\footnotesize]
  ...
  \draw[bmtLaterite, line width=1.1pt, {Stealth[length=2.2mm]}-{Stealth[length=2.2mm]}]
    (40:2.32) -- (40:3.28) node[midway,sloped,below=5pt] {$\times k$};
\end{tikzpicture}
\end{center}
...
% Transformations exactes : rapports 1,4 et 1,3 ; axe x=1,65.
\begin{visualisation}[directe ou indirecte : suivre l'orientation]
\begin{center}
\begin{tikzpicture}[x=.92cm,y=.92cm,font=\footnotesize]
  \begin{scope}[xshift=-3.7cm]
    \draw[bmtGris,line width=.8pt] (0,0)--(1.25,.15)--(.35,1)--cycle;
    \node[bmtGris,below left] at (0,0) {$A$};
    \node[bmtGris,below] at (1.25,.15) {$B$};
    \node[bmtGris,above] at (.35,1) {$C$};
    \draw[bmtAccent,line width=1.15pt]
      (1.75,.15)--(3.5,.36)--(2.24,1.55)--cycle;
    \node[bmtAccent,below] at (1.75,.15) {$A'$};
    \node[bmtAccent,right] at (3.5,.36) {$B'$};
    \node[bmtAccent,above] at (2.24,1.55) {$C'$};
    \draw[bmtLaterite,-{Stealth[length=2mm]}] (1.2,1.42) arc (145:55:.48);
    \node at (1.8,-.48) {similitude directe};
    \node[bmtVetiver] at (1.8,-.82) {même sens de parcours};
  \end{scope}
  \begin{scope}[xshift=2.1cm]
    \draw[bmtVetiver,dashed,line width=.8pt] (1.65,-.2)--(1.65,1.8)
      node[above] {axe};
    \draw[bmtGris,line width=.8pt] (0,0)--(1.15,.15)--(.35,1)--cycle;
    \node[bmtGris,below left] at (0,0) {$A$};
    \node[bmtGris,below] at (1.15,.15) {$B$};
    \node[bmtGris,above] at (.35,1) {$C$};
    \draw[bmtAccent,line width=1.15pt]
      (3.795,0)--(2.3,.195)--(3.34,1.3)--cycle;
    \node[bmtAccent,below right] at (3.795,0) {$A'$};
    \node[bmtAccent,below] at (2.3,.195) {$B'$};
    \node[bmtAccent,above] at (3.34,1.3) {$C'$};
    \draw[bmtLaterite,-{Stealth[length=2mm]}] (1.25,1.43) arc (35:145:.46);
    \node at (1.8,-.48) {similitude indirecte};
    \node[bmtLaterite] at (1.8,-.82) {sens de parcours inversé};
  \end{scope}
\end{tikzpicture}
\end{center}

Une similitude conserve la forme mais pas nécessairement l'orientation. Pour
trancher sans calcul, on parcourt les sommets dans l'ordre $A$, $B$, $C$ puis
$A'$, $B'$, $C'$ : même sens, elle est directe ; sens opposé, elle est
indirecte. L'axe en pointillés rappelle alors la réflexion contenue dans la
forme réduite.
\end{visualisation}
