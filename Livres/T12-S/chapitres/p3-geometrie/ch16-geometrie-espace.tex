...
\begin{tikzpicture}[x=1cm, y=1cm, font=\footnotesize]
  ...
  \draw[bmtLaterite, line width=1.1pt, -{Stealth[length=2.6mm]}]
    (O) -- ++(1.05,0.95) node[anchor=north west] {$\vecv$};
...
% Perspective adaptée des constructions de TCD/Appendix/Geometrie.tex.
\begin{visualisation}[la distance suit la perpendiculaire]
\begin{center}
\begin{tikzpicture}[x=1.08cm,y=1.08cm,font=\footnotesize]
  \coordinate (H) at (.25,.18);
  \coordinate (A) at (.25,2.35);
  \fill[bmtAccent!8]
    (-2,-.55)--(1.55,-.55)--(2.45,.72)--(-1.1,.72)--cycle;
  \draw[bmtAccent,line width=.9pt]
    (-2,-.55)--(1.55,-.55)--(2.45,.72)--(-1.1,.72)--cycle;
  \node[bmtAccent] at (2.05,.37) {$(P)$};
  \draw[bmtLaterite,line width=1.1pt,dashed] (A)--(H);
  \draw[bmtVetiver,line width=1pt,-{Stealth[length=2.3mm]}]
    (H)--(.25,1.38) node[pos=.85,left=4pt] {$\vecn$};
  \draw[bmtEncre] (.25,.18)--(.48,.18)--(.48,.41)--(.25,.41);
  \fill[bmtLaterite] (A) circle (2.3pt) node[above] {$A$};
  \fill[bmtEncre] (H) circle (2pt) node[below left] {$H$};
  \draw[bmtLaterite,decorate,decoration={brace,mirror,amplitude=4pt}]
    (.62,.18)--(.62,2.35) node[midway,right=5pt] {$d(A,(P))=AH$};
\end{tikzpicture}
\end{center}

Le point $H$ est le projeté orthogonal de $A$ sur $(P)$. La distance cherchée
n'est pas mesurée suivant une direction quelconque : c'est la longueur du
plus court segment joignant $A$ au plan, donc $AH$, parallèle au vecteur
normal $\vecn$. La formule précédente calcule précisément cette longueur.
\end{visualisation}
...
% Schémas de position inspirés de TCD/Appendix/Geometrie.tex.
\begin{visualisation}[une droite face à un plan]
\begin{center}
\begin{tikzpicture}[x=.82cm,y=.82cm,font=\scriptsize]
  \begin{scope}[xshift=-5cm]
    \fill[bmtAccent!8] (-1.6,-.45)--(1,-.45)--(1.65,.42)--(-.95,.42)--cycle;
    \draw[bmtAccent] (-1.6,-.45)--(1,-.45)--(1.65,.42)--(-.95,.42)--cycle;
    \draw[bmtLaterite,line width=1pt] (-.9,1.25)--(0,.04);
    \draw[bmtLaterite,line width=1pt,dashed] (0,.04)--(.75,-.968333);
    \fill[bmtEncre] (0,.04) circle (1.8pt) node[right] {$I$};
    \node at (0,-1.35) {sécante};
  \end{scope}
  \begin{scope}
    \fill[bmtAccent!8] (-1.6,-.45)--(1,-.45)--(1.65,.42)--(-.95,.42)--cycle;
    \draw[bmtAccent] (-1.6,-.45)--(1,-.45)--(1.65,.42)--(-.95,.42)--cycle;
    \draw[bmtVetiver,line width=1pt] (-1.2,1.05)--(1.25,1.05);
    \draw[bmtGris,dashed] (-1.2,.65)--(1.25,.65);
    \node at (0,-1.35) {strictement parallèle};
  \end{scope}
  \begin{scope}[xshift=5cm]
    \fill[bmtAccent!8] (-1.6,-.45)--(1,-.45)--(1.65,.42)--(-.95,.42)--cycle;
    \draw[bmtAccent] (-1.6,-.45)--(1,-.45)--(1.65,.42)--(-.95,.42)--cycle;
    \draw[bmtIndigo,line width=1pt] (-1.05,-.12)--(1.05,.2);
    \node at (0,-1.35) {contenue dans le plan};
  \end{scope}
\end{tikzpicture}
\end{center}

Le produit scalaire $\scal{\vecn}{\vecu}$ sépare d'abord la situation de
gauche des deux autres. S'il est non nul, la droite rencontre le plan en un
seul point. S'il est nul, on teste ensuite un point de la droite : selon qu'il
appartient ou non au plan, la droite est contenue dans le plan ou strictement
parallèle.
\end{visualisation}
