...
\begin{tikzpicture}[x=1.15cm, y=1.15cm, font=\footnotesize]
  ...
  \draw[bmtLaterite, decorate, decoration={brace,mirror, amplitude=4pt}]
...
% Construction adaptée de TCD/Barycentre/Lesona.tex.
\begin{visualisation}[regrouper les points pondérés]
\begin{center}
\begin{tikzpicture}[x=1.05cm,y=1.05cm,font=\footnotesize]
  \coordinate (A) at (0,0);
  \coordinate (B) at (5,0);
  \coordinate (C) at (1.15,3.05);
  \coordinate (I) at (2.5,0);
  \coordinate (G) at ($(I)!.5!(C)$);
  \draw[bmtGris,line width=.9pt] (A)--(B)--(C)--cycle;
  \draw[bmtVetiver,dashed,line width=.9pt] (C)--(I);
  \fill[bmtIndigo] (A) circle (2pt) node[below left] {$A(1)$};
  \fill[bmtIndigo] (B) circle (2pt) node[below right] {$B(1)$};
  \fill[bmtIndigo] (C) circle (2pt) node[above] {$C(2)$};
  \fill[bmtVetiver] (I) circle (2.1pt) node[below] {$I(2)$};
  \fill[bmtLaterite] (G) circle (2.4pt) node[right] {$G$};
  \draw[bmtAccent,decorate,decoration={brace,amplitude=3.5pt}]
    ($(I)+(.18,.06)$)--($(G)+(.18,.06)$)
    ;
  \node[bmtAccent] at (3.5,2.15) {$IG=GC$};
\end{tikzpicture}
\end{center}

Pour le système $\{(A;1),(B;1),(C;2)\}$, on remplace d'abord $A$ et $B$
par leur milieu $I$, affecté du coefficient $2$. Les poids de $I$ et $C$
sont alors égaux : $G$ est le milieu de $[IC]$. L'associativité transforme
ainsi une formule à trois points en deux constructions de milieux.
\end{visualisation}
