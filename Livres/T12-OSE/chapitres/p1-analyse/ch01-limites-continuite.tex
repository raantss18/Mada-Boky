...
%  PE 2026, composante ANALYSE (série OSE)
% ===========================================================================
...
de \emph{démontrer} — qu'une équation a une solution, qu'un coût de
production reste maîtrisé au-delà d'un certain volume, qu'une fonction se
prolonge là où elle n'était pas définie.
...
suites, et jusque dans les problèmes de mathématiques financières où il
faudra justifier qu'un taux recherché existe et qu'il est unique.
...
\exo[\niveau{3}] Une entreprise modélise ses coûts de production journaliers
$C(x)$, en milliers d'ariary, pour $x$ unités fabriquées ($x \geq 1$), par
$C(x) = 3x + \dfrac{\sin x}{x}$. Montrer que $\limpinf C(x) $ existe et vaut
la même chose que celle de $3x$, puis interpréter économiquement ce résultat.
\end{entrainement}
...
\textbf{4.} $\dfrac{\sin x}{x} \to 0$ en $+\infty$ (exemple ci-dessus), donc
$C(x) - 3x \to 0$ : le coût de production croît comme $3x$, c'est-à-dire
qu'au-delà d'un certain volume, le terme oscillant devient négligeable devant
le coût variable proprement dit.
\end{corrige}
...
continuité, qui est une propriété locale, en une garantie d'existence — celle,
par exemple, qu'un prix d'équilibre ou un taux recherché existe bel et bien.
...
\exo[\niveau{3}] Une coopérative agricole modélise son bénéfice journalier, en
milliers d'ariary, par $B(x) = -x^{2} + 40x - 305$ pour $x \in \intervalle{0}{15}$
tonnes vendues (au-delà de $15$ tonnes, l'entrepôt ne suit plus). Montrer
qu'il existe un unique volume $x_{0} \in \intervalle{0}{15}$ à partir duquel
le bénéfice devient positif, et l'encadrer à une tonne près.
\end{entrainement}
...
\textbf{3.} $B$ est un polynôme, donc continue sur $\intervalle{0}{15}$.
$B'(x) = -2x+40$, strictement positif sur $\intervalle{0}{15}$ (le sommet de
la parabole est en $x=20$), donc $B$ y est strictement croissante. $B(0) = -305 < 0$
et $B(15) = -225+600-305 = 70 > 0$ : $0$ est bien compris entre $B(0)$ et
$B(15)$, donc le théorème (cas monotone) donne un unique
$x_{0} \in \intervalle{0}{15}$ tel que $B(x_{0}) = 0$. Comme $B(10) = -5 < 0$
et $B(11) = 14 > 0$, on a $10 < x_{0} < 11$ : le seuil de rentabilité se situe
entre $10$ et $11$ tonnes vendues par jour.
\end{corrige}

\begin{notepourlaclasse}
Le troisième exercice se prête à une discussion sur la lecture d'un énoncé de
gestion : le seuil de rentabilité est exactement le type de question que
l'exercice « problème » du baccalauréat OSE pose chaque année, sous une forme
plus développée (voir mathématiques financières, partie V).
...
et dans les pays voisins ; leur provenance est indiquée à chaque fois. Aucun
sujet de baccalauréat OSE examiné pour ce livre ne consacre d'exercice isolé
aux limites et à la continuité seules — le problème y aborde directement une
fonction avec logarithme ou exponentielle. Les énoncés ci-dessous, tous
antérieurs à ces deux notions, proviennent donc du même fonds générique déjà
vérifié pour le livre de série S : ils ne portent que sur des fonctions
polynômes, rationnelles ou irrationnelles, un contenu qui ne dépend pas de la
série.
...
\exoann{Baccalauréat blanc, lycée Philibert Tsiranana, Terminale, 2022-2023}
...
\medskip
\bmtsource{D'autres exercices sur ce thème figurent aux chapitres 5 et 6, où
ils font intervenir le logarithme ou l'exponentielle ; ils y sont signalés
par la mention « limites et continuité ».}
\end{annales}
...
% ===========================================================================
\begin{jecherche}[le seuil de rentabilité d'un atelier]
Un atelier de couture fabrique des uniformes scolaires. Pour $x$ uniformes
produits par jour ($x \geq 1$), le coût moyen unitaire, en milliers d'ariary,
est modélisé par
\[
  C_{m}(x) = 6 + \frac{40}{x}.
\]
Les $40$ représentent le coût fixe journalier de l'atelier (loyer, machines),
réparti sur les $x$ uniformes ; les $6$ représentent le coût variable, en
tissu et en main-d'œuvre, pour chaque uniforme supplémentaire.

\begin{enumerate}[label=\textbf{\arabic*.}, leftmargin=*, itemsep=0.3em]
  \item Vérifier que $C_{m}(1) = 46$, et interpréter ce nombre : que
        coûterait un unique uniforme, s'il fallait absorber seul le coût
        fixe de la journée ?
  \item Déterminer $\lim_{x \to +\infty} C_{m}(x)$ et interpréter ce nombre
        pour l'atelier.
  \item Montrer que $C_{m}$ est strictement décroissante sur
        $\left[1 ; +\infty\right[$.
  \item Chaque uniforme est vendu $7\,000$ ariary. On note
        $M(x) = 7 - C_{m}(x)$ la marge unitaire, en milliers d'ariary, pour
        $x$ uniformes produits. Justifier que $M$ est continue et
        strictement croissante sur $\left[1 ; +\infty\right[$.
  \item Montrer que l'équation $M(x) = 0$ admet une unique solution
        $x_{0}$ dans $\left[1 ; +\infty\right[$, puis la déterminer.
        Interpréter $x_{0}$ pour l'atelier.
\end{enumerate}
\end{jecherche}

\begin{corrige}[le seuil de rentabilité d'un atelier]
\textbf{1.} $C_{m}(1) = 6 + 40 = 46$ : produire un seul uniforme dans la
journée reviendrait à $46\,000$ ariary, puisqu'il faudrait alors absorber à
lui seul tout le coût fixe.

\textbf{2.} $\dfrac{40}{x} \to 0$ quand $x \to +\infty$, donc
$\lim_{x \to +\infty} C_{m}(x) = 6$. À mesure que la production augmente, le
coût fixe se dilue sur un grand nombre d'uniformes et le coût moyen se
rapproche du seul coût variable, $6\,000$ ariary, sans jamais l'atteindre.

\textbf{3.} $C_{m}'(x) = -\dfrac{40}{x^{2}} < 0$ pour tout $x \geq 1$ : $C_{m}$
est strictement décroissante sur $\left[1;+\infty\right[$.

\textbf{4.} $M$ est continue sur $\left[1;+\infty\right[$ comme différence
d'une constante et d'une fonction continue (somme d'une constante et d'un
quotient à dénominateur non nul). $M'(x) = -C_{m}'(x) = \dfrac{40}{x^{2}} > 0$ :
$M$ est strictement croissante sur $\left[1;+\infty\right[$.

\textbf{5.} $M(1) = 7 - 46 = -39 < 0$ et $\lim_{x\to+\infty} M(x) = 7 - 6 = 1 > 0$.
$M$ étant continue et strictement croissante sur $\left[1;+\infty\right[$, le
théorème des valeurs intermédiaires (cas monotone) donne un unique $x_{0}$ tel
que $M(x_{0}) = 0$. On résout directement :
\[
  7 - 6 - \frac{40}{x_{0}} = 0
  \iff \frac{40}{x_{0}} = 1
  \iff x_{0} = 40 .
\]
En deçà de $40$ uniformes produits par jour, la marge unitaire est négative :
l'atelier vend à perte. Au-delà, chaque uniforme supplémentaire est
rentable.
\end{corrige}
