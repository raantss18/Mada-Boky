...
\glo{Arrangement}{12}{Liste \emph{ordonnée} de $p$ objets distincts choisis
...
\glo{Cas favorables}{13}{Nombre d'issues qui réalisent l'événement étudié.
...
\glo{Coefficient multiplicateur}{14}{Nombre $1+t$ par lequel on multiplie
une valeur pour lui appliquer une évolution de taux $t$.}

\glo{Combinaison}{12}{Partie de $p$ objets choisis parmi $n$, \emph{sans}
ordre. Leur nombre est $\combi{p}{n} = \frac{n!}{p!(n-p)!}$ ; c'est le
décompte d'un tirage simultané.}

\glo{Convergence}{10}{Une suite converge lorsqu'elle admet une limite
\emph{finie}. Tendre vers l'infini, c'est diverger.}
...
\glo{Dénombrement}{12}{Art de compter le nombre de résultats possibles d'une
...
\glo{Divergence}{10}{Absence de limite finie : la suite tend vers l'infini,
ou n'a pas de limite du tout.}

\glo{Droite d'ajustement}{14}{Droite qui résume un nuage de points et permet
...
\glo{Équiprobabilité}{13}{Situation où toutes les issues ont la même chance.
Signalée par « indiscernables au toucher » ou « tirages équiprobables ».}

\glo{Événement}{13}{Partie de l'univers. Il est \emph{certain} s'il est
l'univers entier, \emph{impossible} s'il est vide.}

\glo{Événement contraire}{13}{Événement $\evtc{A}$ réalisé exactement quand
...
\glo{Extrapolation}{14}{Estimation faite au-delà des valeurs observées. Elle
suppose que la tendance se poursuit ; ce n'est jamais une certitude.}

% ---------------------------------------------------------------------------
\glolettre{F}

\glo{Factorielle}{12}{$n! = n\times(n-1)\times\cdots\times1$, avec la
convention $0! = 1$.}
...
\glo{Mayer (méthode de)}{14}{Ajustement d'un nuage par la droite joignant
les points moyens des deux moitiés de la série.}

\glo{Moyenne}{14}{Somme des valeurs divisée par leur nombre.}
...
\glo{Nuage de points}{14}{Représentation graphique d'une série statistique
double, un point par couple observé.}

% ---------------------------------------------------------------------------
\glolettre{P}

\glo{Permutation}{12}{Rangement de $n$ objets dans un ordre. Leur nombre est
$n!$.}

\glo{Point fixe}{10}{Valeur $\alpha$ telle que $g(\alpha) = \alpha$ : si un
terme de la suite l'atteint, tous les suivants y restent.}

\glo{Point moyen}{14}{Point $\pmoyen(\moy x\,;\,\moy y)$ du nuage. La droite
...
\glo{Probabilité}{13}{Nombre compris entre $0$ et $1$ mesurant la chance
qu'un événement se réalise.}

% ---------------------------------------------------------------------------
\glolettre{R}

\glo{Raison}{11}{Nombre $r$ que l'on ajoute (suite arithmétique) ou nombre
$q$ par lequel on multiplie (suite géométrique) pour passer d'un terme au
suivant.}

\glo{Récurrente (suite)}{10}{Suite définie par un premier terme et une
...
\glo{Série statistique double}{14}{Liste de couples
$\left(x_{i}\,;\,y_{i}\right)$ observés sur une même population.}

\glo{Suite arithmétique}{11}{Suite vérifiant $u_{n+1} = u_{n}+r$.}

\glo{Suite auxiliaire}{11}{Suite $v_{n} = u_{n}-\alpha$ introduite pour
ramener une suite récurrente à une suite géométrique. C'est la méthode de
l'exercice 1 du baccalauréat.}

\glo{Suite géométrique}{11}{Suite vérifiant $u_{n+1} = q\,u_{n}$.}
...
\glo{Taux d'évolution}{14}{Quotient $\frac{V_{1}-V_{0}}{V_{0}}$, exprimé en
pourcentage.}

\glo{Tirage simultané}{12}{Tirage « d'un seul coup », où l'ordre ne compte
pas : on dénombre avec $\combi{p}{n}$.}

\glo{Tirage successif}{12}{Tirage un par un, où l'ordre compte : avec remise
...
\glo{Univers}{13}{Ensemble $\Omega$ de tous les résultats possibles d'une
...
\glo{Variation (sens de)}{5}{Croissance ou décroissance d'une fonction ou
d'une suite sur un intervalle donné.}
