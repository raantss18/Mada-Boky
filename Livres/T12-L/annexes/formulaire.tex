...
\hline
Définition & $u_{n+1} = u_{n}+r$ & $u_{n+1} = q\,u_{n}$\\
Terme général & $u_{n} = u_{0}+nr$ & $u_{n} = u_{0}\,q^{n}$\\
 & $u_{n} = u_{p}+(n-p)r$ & $u_{n} = u_{p}\,q^{\,n-p}$\\
Somme & $\left(\text{nb de termes}\right)
        \times\dfrac{\text{premier}+\text{dernier}}{2}$ &
       $\text{premier}\times\dfrac{1-q^{\text{nb de termes}}}{1-q}$\\
Limite & $+\infty$ si $r>0$, $-\infty$ si $r<0$ &
       $0$ si $\abs q<1$ ; $+\infty$ si $q>1$ ; pas de limite si
       $q \leq -1$\\
\hline
\end{tabular}
\end{center}

\textbf{Nombre de termes :} de $u_{0}$ à $u_{n}$, il y en a $n+1$ ; de
$u_{p}$ à $u_{n}$, il y en a $n-p+1$.

\textbf{Suite auxiliaire.} Si $u_{n+1} = a\,u_{n}+b$ et
$v_{n} = u_{n}-\alpha$ où $\alpha$ est le point fixe, alors
$\left(v_{n}\right)$ est géométrique de raison $a$.

\textbf{Passage $\ln$ / $\exp$.} Si $\left(u_{n}\right)$ est géométrique de
raison $q>0$, alors $\left(\ln u_{n}\right)$ est arithmétique de raison
$\ln q$. Si $\left(u_{n}\right)$ est arithmétique de raison $r$, alors
$\left(e^{u_{n}}\right)$ est géométrique de raison $e^{r}$.
