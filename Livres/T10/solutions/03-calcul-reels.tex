...
\begin{enumerate}
  ...
  \item $\dfrac{2^{-3}}{4^{-1}}=\dfrac{1/8}{1/4}=\dfrac12$
  \item $\dfrac{(xy)^4}{x^2y^6}=\dfrac{x^2}{y^2}$ ($x,y\ne0$)
\end{enumerate}
\end{multicols}
...
\begin{enumerate}
  ...
  \item $\abs{2x-1}\geq\abs{x+3}\Leftrightarrow
        (2x-1)^2\geq(x+3)^2\Leftrightarrow
        3x^2-10x-8\geq0\Leftrightarrow(3x+2)(x-4)\geq0$.
        Solution~: $]-\infty,-2/3]\cup[4,+\infty[$.
\end{enumerate}
...
\begin{enumerate}
  ...
  \item $\dfrac1x+\dfrac1{x+1}=\dfrac{2x+1}{x(x+1)}$
        ($x\ne0,-1$)
  \item $\dfrac{x}{x-1}-\dfrac1{x+1}
        =\dfrac{x^2+1}{(x-1)(x+1)}$ ($x\ne-1,1$)
  \item $\dfrac{x+2}{x-1}\times\dfrac{x-1}{x+3}
        =\dfrac{x+2}{x+3}$ ($x\ne1,-3$)
  \item $\dfrac{(x+1)^2}{x+1}=x+1$ ($x\ne-1$)
\end{enumerate}
\end{multicols}
...
\begin{enumerate}
  \item $a\geq0$~: $|a|=a=|-a|$.\quad $a<0$~: $|a|=-a$ et $|-a|=-a=|a|$.
  ...
  \item L'in\'egalit\'e triangulaire donne
        $|a|=|(a-b)+b|\leq|a-b|+|b|$, donc $|a|-|b|\leq|a-b|$.
        En \'echangeant $a$ et $b$, on obtient aussi
        $|b|-|a|\leq|a-b|$. Ainsi $\bigl||a|-|b|\bigr|\leq|a-b|$.
\end{enumerate}
...
\begin{enumerate}
  \item $\begin{cases}x+1>0\\2x-3\leq5\end{cases}\Leftrightarrow\begin{cases}x>-1\\x\leq4\end{cases}$
        Solution~: $]-1,4]$.
  \item $\begin{cases}3x-2\geq1\\x+4<8\end{cases}
        \Leftrightarrow\begin{cases}x\geq1\\x<4\end{cases}$.
        Solution~: $[1,4[$.
  \item $\begin{cases}\abs{x}\leq3\\x\geq1\end{cases}$~:
        $-3\leq x\leq3$ et $x\geq1$. Solution~: $[1,3]$.
  \item $\begin{cases}x^2\geq1\\x<3\end{cases}$~:
        $x\leq-1$ ou $x\geq1$, avec $x<3$.
        Solution~: $]-\infty,-1]\cup[1,3[$.
\end{enumerate}
...
\begin{enumerate}
  ...
  \item $1{,}4^3=2{,}744<3$ et $1{,}5^3=3{,}375>3$.
        Donc $1{,}4<\sqrt[3]{3}<1{,}5$.
\end{enumerate}
...
\begin{enumerate}
  ...
  \item Appliquons la premi\`ere in\'egalit\'e \`a $\sqrt a$ et $\sqrt b$~:
        $a+b\geq2\sqrt{ab}$, soit $\sqrt{ab}\leq(a+b)/2$.
  \item Pour $x>0$, appliquer le r\'esultat pr\'ec\'edent \`a
        $a=x$ et $b=4/x$~: $(x+4/x)/2\geq\sqrt4=2$.
        Donc $x+4/x\geq4$.
  \item L'\'egalit\'e a lieu si $x=4/x$, soit $x=2$ puisque $x>0$.
\end{enumerate}
...
\begin{enumerate}
  ...
  \item Plus g\'en\'eralement, pour $x\geq-1$ et $n\geq1$~:
        ...
        Pour $-1\leq x<0$, on a
        $(1+x)^3-(1+3x)=x^2(3+x)\geq0$.
  \item Pour $x=0{,}1$, les valeurs pour $n=1,2,3,4,5$ sont
        $1{,}1$~; $1{,}21$~; $1{,}331$~; $1{,}4641$~; $1{,}61051$.
        Chacune est au moins \'egale \`a $1+0{,}1n$.
  \item Avec l'in\'egalit\'e admise dans l'\'enonc\'e,
        $(1{,}05)^{10}\geq1+10(0{,}05)=1{,}5$.
\end{enumerate}
...
\begin{enumerate}
  ...
  \item Supposons $r=\sqrt2+\sqrt3\in\Q$.
        Comme $r>0$, on a $1/r=(\sqrt3-\sqrt2)\in\Q$.
        Alors $\sqrt2=(r-1/r)/2\in\Q$, contradiction.
        Donc $\sqrt2+\sqrt3\notin\Q$.
\end{enumerate}
...
\begin{enumerate}
  \item En utilisant $\sqrt2\approx1{,}41421356$, on compl\`ete~:
        \begin{center}
        \begin{tabular}{|c|c|c|}\hline
        \rowcolor{BN!10} Fraction & Valeur d\'ecimale & Erreur absolue \\ \hline
        $1$ & $1$ & $0{,}414214$ \\
        $3/2$ & $1{,}5$ & $0{,}085786$ \\
        $7/5$ & $1{,}4$ & $0{,}014214$ \\
        $17/12$ & $1{,}416667$ & $0{,}002453$ \\
        $41/29$ & $1{,}413793$ & $0{,}000420$ \\ \hline
        \end{tabular}
        \end{center}
  \item $\sqrt2<10/7\approx1{,}428571<1{,}5$ et $7\leq10$.
  \item Oui~: les troncatures d\'ecimales de $\sqrt2$ sont rationnelles
        et leur distance \`a $\sqrt2$ peut devenir arbitrairement petite.
  \item Si $a<b$, alors $a<(a+b)/2<b$. En prenant chaque fois le milieu
        entre $a$ et le dernier milieu obtenu, on construit une infinit\'e
        de r\'eels distincts dans $]a,b[$.
\end{enumerate}
...
\begin{enumerate}
  ...
  \item $|a^2|=a^2$, car $a^2\geq0$.\hfill$\square$
  \item $|a|=\sqrt{a^2}$, car les deux membres sont positifs
        et ont pour carr\'e $a^2$.\hfill$\square$
  \item Si $|a|\leq M$ et $|b|\leq N$, alors $M,N\geq0$ et
        $|ab|=|a||b|\leq MN$.\hfill$\square$
\end{enumerate}

%────────────────────────────────────────────────────────────────
\solutiontitle{3}{37}{Approximation de $\pi$}

Archim\`ede obtint $\dfrac{223}{71}<\pi<\dfrac{22}{7}$.

\begin{enumerate}
  \item $223/71\approx3{,}140845<\pi\approx3{,}141593$
        et $22/7\approx3{,}142857>\pi$.
  \item Les chiffres garantis apr\`es la virgule sont $1$ et $4$~:
        $\pi=3{,}14\ldots$ d'apr\`es cet encadrement.
  \item $|22/7-\pi|\approx0{,}001264$, tandis que
        $|3{,}14-\pi|\approx0{,}001593$.
        L'approximation $22/7$ est l\'eg\`erement meilleure.
\end{enumerate}
...
\begin{enumerate}
  ...
  \item $\exists x\in[0,+\infty[, \sqrt{x}<0$~: \textbf{Fausse}.
        N\'egation~: $\forall x\in[0,+\infty[, \sqrt{x}\geq0$. Vraie.
...
\begin{enumerate}
  ...
  \item Si $a\ne0$, $\Delta>0$ est une \textbf{CNS} pour que
        $ax^2+bx+c=0$ ait deux racines r\'eelles distinctes.\hfill$\square$
\end{enumerate}
...
\begin{enumerate}
  ...
  \item Sous l'hypoth\`ese $a>b$, on a
        $a^2-b^2=(a-b)(a+b)>0$ si et seulement si $a+b>0$.
  \item $a^2>b^2\Rightarrow|a|>|b|$, pas forc\'ement $a>b$.
        Contre-exemple~: $a=-3, b=2$~: $9>4$ mais $-3<2$.
  \item Le tableau de v\'erit\'e des quatre cas demand\'es est~:
\begin{center}\renewcommand{\arraystretch}{1.3}
